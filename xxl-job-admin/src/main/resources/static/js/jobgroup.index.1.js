$(function() {
	...
	$('.add').on('click', function(){
        $.post({
            url : base_url + "/jobbusgroup/selectAll",
            data : {
            },
            dataType : "json",
            success : function(data){
                if (data.code == 200) {
                   var jobGroup = $('#jobGroupOfter');
                    jobGroup.empty();
                    var jobgroupList = data.content;
                    for (d in jobgroupList) {
                　　　　jobGroup.append("<option value='"+jobgroupList[d].id+"'>"+jobgroupList[d].name+"</option>");
                　　}
                } else {
