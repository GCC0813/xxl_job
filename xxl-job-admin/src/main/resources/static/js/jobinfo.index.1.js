$(function() {
	...
	$(".add").click(function(){
		...
		var jobGroup4Select = $("#jobGroup");
        var jobGroupOfter = $('#jobGroupOfter');
        jobGroupOfter.empty();
        // 刷新执行器选项框
        jobGroupOfter.html(jobGroup4Select.html());
        // 去除全部选项
        jobGroupOfter.find("option[value='0']").remove();
	...
	$("#job_list").on('click', '.job_copy',function() {
	    ...
	    // 业务组变更时刷新执行器搜索选项数据
    	$('#jobBusGroup').on('change', function(){
    		var busId = $(this).val();
    		$.ajax({
                type : 'POST',
                url : base_url + "/jobinfo/findByBusId",
                data : {
                    "id" : busId
                },
                dataType : "json",
                success : function(data){
                    if (data.code == 200) {
                    	...
                    	jobGroup.empty();
                    	var jobgroupList = data.content;
                    	jobGroup.append("<option value='0'>全部</option>");
                    	for (d in jobgroupList) {
                	　　　　jobGroup.append("<option value='"+jobgroupList[d].id+"'>"+jobgroupList[d].title+"</option>");
                	　　}
                    }
                }
            });
